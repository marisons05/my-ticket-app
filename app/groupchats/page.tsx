  ...
  return (
    <>
      <style>{`
        ...
        .chat-row:hover { background: rgba(255,255,255,0.07) !important; }
      ...
      <main style={{
        ...
        <div style={{ position: 'relative', zIndex: 10, padding: '48px 24px 80px', maxWidth: 640, margin: '0 auto' }}>
          <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 11, letterSpacing: 4, marginBottom: 6, textTransform: 'uppercase' }}>Community</p>
          <h1 style={{ color: 'white', fontSize: 32, fontWeight: 900, marginBottom: 32, letterSpacing: 1 }}>Groupchats</h1>

          {!loaded ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{ height: 80, borderRadius: 14, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', opacity: 1 - i * 0.25 }} />
              ))}
            </div>
          ) : chats.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', animation: 'fadeUp 0.5s ease both' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.38)', fontSize: 14, maxWidth: 280, margin: '0 auto 24px', lineHeight: 1.6 }}>
                Buy a ticket to an event and you&apos;ll be added to that event&apos;s groupchat automatically.
              </p>
              <Link
                href="/events"
                style={{ background: 'linear-gradient(135deg, #7c3aed, #ff6b9d)', color: 'white', padding: '12px 28px', borderRadius: 12, textDecoration: 'none', fontSize: 14, fontWeight: 700, display: 'inline-block' }}
              >
                Browse Events
              </Link>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, animation: 'fadeUp 0.5s ease both' }}>
              {chats.map(chat => (
                ...
                >
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, rgba(124,58,237,0.5), rgba(255,107,157,0.5))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>
                    🎵
                  </div>
